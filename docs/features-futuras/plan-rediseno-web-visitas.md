...
**Fase 5 — Wizard de 3 pasos + barra de búsqueda compuesta + `Activity`/`ViewTransition`**: mayor superficie de cambio. Nuevo `componentes/PasoWizard.tsx` (con `.nav-pills`/badges), nuevo `lib/useFocoAlCambiar.ts`, `NuevaCita.tsx` reestructurado (3 pasos, filtrado de errores por paso reutilizando `esquemaNuevaCita().safeParse()` completo sin sub-esquemas). **Mayor riesgo**: `e2e/visitas.spec.ts` necesita reescritura sustancial (flujo cambia de 1 click a 2 "Continuar" + 1 "Confirmar") — aislar en su propio commit, correr Playwright en ambos proyectos (`escritorio`, `movil`).
