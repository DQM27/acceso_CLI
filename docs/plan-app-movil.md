...
- **Selector de modo de búsqueda en Activos (2026-09-01, mismo día).** Con muchos activos a la vez (ej. 100), buscar sólo por texto no alcanza: el mismo campo no puede a la vez buscar en el catálogo (para entrada) y filtrar los activos (para salida). Se agregó un selector de tres opciones arriba del campo — Entrada / Salida: nombre / Salida: gafete — que decide cuál de las tres cosas hace el campo, con el color del campo cambiando entre modo Entrada y modo Salida como aviso visual. Salida: nombre y Salida: gafete van separados (y no un solo modo "salida" como en `SalidaModal.tsx`) porque la búsqueda de texto libre de Rust ya mezcla nombre/cédula con gafete en el mismo `OR` — buscar "7" como gafete también traería cualquier cédula que lo contenga, ruidoso con muchos activos. Se agregó `ModoBusquedaActivos` (`NombreCedula`/`Gafete`) a `Nucleo::listar_ingresos_activos` en `mobile/rust-core/src/lib.rs` para que el filtro por gafete sea exacto del lado de Rust (`FiltroIngresosActivos.gafete_numero`), no una búsqueda de texto libre resuelta a medias en el teléfono. **Corrección al mismo día:** el punto anterior decía que el modo gafete de mobile confirmaba cada coincidencia por separado con diálogo, a diferencia de `SalidaModal.tsx`. El usuario pidió explícitamente igualar el comportamiento de desktop, así que se agregó: el campo acepta varios números de gafete separados por coma ("2, 25, 85"), muestra una vista previa de a quién le corresponde cada uno (o "Sin ingreso activo"), y un solo botón registra la salida de todos los que sí tienen ingreso activo, sin diálogo por persona — la vista previa con nombres hace de confirmación, igual que en desktop. También se corrigió que Salida: nombre y Salida: gafete precargaban toda la lista de activos con el campo vacío; ahora, al ser buscadores (la lista para recorrer sigue siendo la pestaña Entrada), no consultan nada hasta que se escribe algo.
