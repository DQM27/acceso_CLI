...
- [x] **Runbook de base local dañada (2026-09-17).** `docs/recuperacion-sitio-local.md` -- aceptaste perder auditoría/incidentes/cola pendiente; queda anotado "a valorar más adelante". Hallazgo de seguridad importante ahí: el secreto de dispositivo vive en la misma carpeta que la base, así que restaurar un sitio requiere pasar por `admin-provision-device` (mismo camino que un dispositivo nuevo), no un atajo.
