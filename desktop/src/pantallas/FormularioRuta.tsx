...
import { crearRuta, crearRutasRango } from "../api";
...
export default function FormularioRuta({
  ...
  async function alGuardar(valores: ValoresFormulario) {
    try {
      if (valores.modo === "individual") {
        await crearRuta(Number(valores.numero));
      } else {
        await crearRutasRango(Number(valores.desde), Number(valores.hasta));
      }
      onGuardado();
    } catch (error) {
  ...
  return (
    <Modal titulo="Nueva ruta" onCerrar={onCerrar}>
      <form
