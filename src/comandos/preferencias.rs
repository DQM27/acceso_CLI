//! Persistencia de preferencias de `--comandos` (hoy sólo columnas visibles
//! por tabla). Archivo propio (`comandos-preferencias.conf`), independiente
...
const FILE_NAME: &str = "comandos-preferencias.conf";
...
mod tests {
    ...
    fn ruta_temporal(sufijo: &str) -> PathBuf {
        std::env::temp_dir().join(format!(
            "comandos-preferencias-test-{}-{sufijo}.conf",
            std::process::id()
        ))
    }
